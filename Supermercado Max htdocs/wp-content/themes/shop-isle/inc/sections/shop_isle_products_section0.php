...
echo '<section id="latest" class="module module-small">';
...
// ID de la categoria que se ve en la URL
$shop_isle_products_category  = 128;

 echo '<script>console.log('.$shop_isle_products_category.')</script>';
...
elseif ( isset( $shop_isle_products_category ) && ! empty( $shop_isle_products_category ) && ( $shop_isle_products_category != '-' ) ) :
	...
	if ( $shop_isle_latest_loop->have_posts() ) :
		...
		if ( function_exists( 'wc_get_page_id' ) ) {
			echo '<a href="' . esc_url( get_permalink( wc_get_page_id( 'shop' ) ) ) . '" class="btn btn-b btn-round">' . apply_filters( 'shop_isle_see_all_products_label', __( 'Ver todos los productos de la tienda', 'shop-isle' ) ) . '</a>';
		} elseif ( function_exists( 'woocommerce_get_page_id' ) ) {
			echo '<a href="' . esc_url( get_permalink( woocommerce_get_page_id( 'shop' ) ) ) . '" class="btn btn-b btn-round">' . apply_filters( 'shop_isle_see_all_products_label', __( 'Ver todos los productos de la tienda', 'shop-isle' ) ) . '</a>';
		}
		echo '</div>';
		echo '</div>';
